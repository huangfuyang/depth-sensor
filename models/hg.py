...
class HourglassNet(nn.Module):
  def __init__(self, nStack, nModules, nFeats):
    ...
    self.nModules = nModules
    self.nFeats = nFeats
    self.conv1_ = nn.Conv2d(1, 64, bias = True, kernel_size = 3, stride = 2, padding = 1)
    ...
    for i in range(self.nStack):
      ...
      _lin_.append(lin)
      _tmpOut.append(nn.Conv2d(self.nFeats, JOINT_LEN, bias = True, kernel_size = 1, stride = 1))
      if i < self.nStack - 1:
        _ll_.append(nn.Conv2d(self.nFeats, self.nFeats, bias = True, kernel_size = 1, stride = 1))
        _tmpOut_.append(nn.Conv2d(JOINT_LEN, self.nFeats, bias = True, kernel_size = 1, stride = 1))
